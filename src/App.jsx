...
// NEW IMPORT - Meal Swipe Game (now integrated)
...
const NutritionApp = () => {
  ...
  // NEW: Track if user has completed the meal swipe game
  const [hasCompletedSwipeGame, setHasCompletedSwipeGame] = useState(false);
  ...
  // NEW: Check if user has meals to swipe through
  ...
  return (
    <div className={`min-h-screen bg-gray-100 ${isMobile ? 'py-4' : 'py-8'}`}>
      <div className={`container mx-auto ${isMobile ? 'px-3' : 'px-4'}`}>
        ...
        {/* NEW: Integrated Meal Swipe Game - Shows when user has meals and hasn't completed it */}
        ...
        {/* Show meal trackers only after swipe game is complete OR if no meals to swipe */}
        {(hasCompletedSwipeGame || !hasMealsToSwipe() || !userProfile.firstName || !userProfile.gender) && (
        ...
        {/* Show summary and charts only after swipe game completion */}
        {(hasCompletedSwipeGame || !hasMealsToSwipe() || !userProfile.firstName || !userProfile.gender) && (
          <>
